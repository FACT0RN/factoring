...
def yafu_factor_driver(n):
  ...
  tmp = []
  try:
     parse = subprocess.run([YAFU_BIN,"-one","-plan", "custom","-pretest_ratio","0.32", "-threads",YAFU_THREADS,"-lathreads",YAFU_LATHREADS, "-ggnfs_dir", MSIEVE_BIN, "-xover", "110", "-snfs_xover", "108",  str(n)], timeout=int(YAFU_TIMEOUT), stdout=subprocess.PIPE)
